...
from game.state import GameState, RACE_TARGET_KWH
import ui.colors as C
import ui.fonts as F
import ui.layout as L


_MILESTONES = [0, 50, 100, 150, int(RACE_TARGET_KWH)]
...
def draw(surf: pygame.Surface, state: GameState) -> list[pygame.Rect]:
    ...
    # Rail line
    pygame.draw.line(surf, (70, 100, 170), (L.TRACK_X0, L.TRACK_Y), (L.TRACK_X1, L.TRACK_Y), 3)

    # Milestone markers
    for kwh in _MILESTONES:
        x = L.kwh_to_x(kwh, RACE_TARGET_KWH)
        is_target = (kwh == RACE_TARGET_KWH)
        ...
        lbl = F.get('tiny').render(f'{int(kwh)} kWh', True, color)
        surf.blit(lbl, (x - lbl.get_width() // 2, L.TRACK_Y + 16))

    # Goal star
    x_goal = L.kwh_to_x(RACE_TARGET_KWH, RACE_TARGET_KWH)
    ...
    for i in range(len(state.players) - 1, -1, -1):
        ...
        x = L.kwh_to_x(player.banked_kwh, RACE_TARGET_KWH)
        y = L.TRACK_Y
    ...
    token_rects.reverse()  # index matches player index
